...
#curl -H "Accept: application/json" -X POST -d "[{\"auth_key\":\"$ENCAP_PASSWORD\"}]" http://localhost:$RUNSERVER_PORT/juniper/showroute/ > /dev/null
curl -H "Accept: application/json" -X POST -d "{\"auth_key\":\"$ENCAP_PASSWORD\"}" http://$RUNSERVER_IP:$RUNSERVER_PORT/juniper/showroute/ > /dev/null
